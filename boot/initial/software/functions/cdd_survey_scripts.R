...
### Time-stamp: <2024-04-25 12:53:03 a23579>
###
### Created: 2016??
...
single_cpue_plot_cohort <- function(cpue)
{
    ...
    for(i in 1:length(cpue)) {
        ...
        suppressWarnings(plot(t.[, 4], t.[, 3], type = "null",
                              xlab = "Cohort", ylab = "Log-mean-standardised index"))
        for(j in sort(unique(t.[, 2]))) {
            lines(t.[t.[, 2] == j, 4], t.[t.[, 2] == j, 3], col = "red")
            text (t.[t.[, 2] == j, 4], t.[t.[, 2] == j, 3],
                  as.character(t.[t.[, 2] == j, 2]), col = "black", cex = 0.6)
        }
...
single_cpue_plot_year <- function(cpue)
{
    ...
    for(i in 1:length(cpue)) {
        ...
        suppressWarnings(plot(t.[, 4], t.[, 3], type = "null",
                              xlab = "Year", ylab = "Log-mean-standardised index"))
        for(j in sort(unique(t.[, 2]))) {
            lines(t.[t.[, 2] == j, 4], t.[t.[, 2] == j, 3], col = "red")
            text (t.[t.[, 2] == j, 4], t.[t.[, 2] == j, 3],
                  as.character(t.[t.[, 2] == j, 2]), col = "black", cex = 0.6)
    ...
    for(i in 1:length(cpue)) {
        ...
        if(nages > 2) {
            ...
            t. <- cbind(rep(yrange, each = max(arange) - min(arange) + 1), arange, catch,
                        rep(effort, each = max(arange) - min(arange) + 1))
            cohort <- t.[, 1] - t.[, 2]
            t. <- cbind(t., cohort)
            tdf <- as.data.frame(t.)

            ## suppressWarnings(plot(tdf[, 1], log(tdf[, 3] / tdf[, 4]), type = "n",
            ##                  xlab = "year of survey", ylab = "log-abundance index"))
            plot(tdf[, 1], log(tdf[, 3] / tdf[, 4]), type = "n",
                 xlab = "Year", ylab = "log-abundance index")
            uc <- sort(unique(tdf[, 5]))
            for(j in 1:length(uc)) {
                v. <- tdf[tdf[, 5] == uc[j], ]
                if(length(v.)>1) {
                    lines(v.[, 1], log(v.[, 3] / v.[, 4]), col = "red")
                    text(v.[, 1], log(v.[, 3] / v.[, 4]),
                         as.character(v.[, 2]), col = "black", cex = 0.6)
                }
            }
            title(main = cpue[[i]]@name)
        }
...
single_catchcurvegrad_index <- function(cpue, agerange)
{
    ...
    for(i in 1:length(cpue)) {
        ...
        if(nages > 2) {
            ...
            logindrat <- cpu
            logindrat[1:nrow(cpu) - 1, 1:ncol(cpu) - 1] <- -log(cpu[1:nrow(cpu) - 1,
                                                                    1:ncol(cpu) - 1] /
                                                                cpu[ - 1, - 1])
            ...
            res <- c(0, 0)
            suppressWarnings(plot(tdg.[, 4], -tdg.[, 3], type = "null",
                                  xlab = "cohort", ylab = "negative gradient"))
            for(j in 1:length(uc)) {
            ...
            lines(res)
            points(res)
            title(main = paste(cpue[[i]]@name, "- ages", agerange[[i]][1],
                               "to", agerange[[i]][2], sep = " "))
            reslist[[i]] <- res
        }
    }
...
corplotswithin_index <- function(cpue, fl)
{
    ...
    if(nages > 1) {
        for(i in arange[1:(length(arange) - 1)]) {
            ...
            pred.w.plim <- predict(MASS::rlm(d2.~d1.), newf, interval = "prediction")
            pred.w.clim <- predict(MASS::rlm(d2.~d1.), newf, interval = "confidence")
            ...
            abline(MASS::rlm(d2.~d1., maxit = 50), lty = 3, xlim = xl, ylim = yl)
            ## print(length(d[, 1]))
            ## print()
            if(is.finite(dtt[length(dtt[, 1]), 1]) & is.finite(dtt[length(dtt[, 2]), 2]))
            {
                text(d[length(d[, 1]), 1], d[length(d[, 2]), 2], label = "[  ]",
                     font = 2, col = "black", cex = 1.0)
            }
            ## print(d)
            ...
            text(d4., y = max(yl) - 0.5, label = paste("cor = ",
                                                       substr(as.character(corr(d)), 1, 5)), col = 1)
        }
    }
    par(def.par)
}
...
catchcurve <- function(stock, agerange = c(NULL, NULL))
{
    ...
    if(nages > 2) {
        ...
        suppressWarnings(plot(tdf[, 1], log(tdf[, 3]), type = "null",
                              xlab = "year of catch", ylab = "log-catch"))
...
plotsbetween_index <- function(cpue, fl1, fl2)
{
    ...
    for(i in arange[1:length(arange)]) {
        ...
        dd <- cbind(d4., d2.)
        print(dd)
        plot(d, type = "n", xlab = "Year", ylab = "Log-numbers",
             ylim = c(min(d1., d2.), max(d1., d2.)), xlim = c(min(d3., d4.), max(d3., d4.)))
